...
// Import axis definitions from multiaxis_control.h
#include "motion/multiaxis_control.h"

// *****************************************************************************
// CNC Machine Settings (GRBL v1.1f Compatible)
// *****************************************************************************

typedef struct
{
    // $100-$103: Steps per millimeter
    float steps_per_mm[NUM_AXES];
    
    // $110-$113: Maximum rate (mm/min)
    float max_rate[NUM_AXES];
    
    // $120-$123: Acceleration (mm/sec²)
    float acceleration[NUM_AXES];
    
    // $130-$133: Maximum travel (mm)
    float max_travel[NUM_AXES];
    
    // $11: Junction deviation (mm) - Controls cornering speed
    float junction_deviation;
    
    // $12: Arc tolerance (mm) - G2/G3 interpolation accuracy
    float arc_tolerance;
    
    // Advanced motion parameters
    float jerk_limit;                // Jerk limit (mm/sec³) for S-curves
    float minimum_planner_speed;     // Minimum junction speed (mm/min)
...
// *****************************************************************************
// Velocity Profile Structures
// *****************************************************************************

/*! \brief Trapezoidal/S-curve velocity profile
 *
 *  Describes acceleration, cruise, and deceleration phases.
 *  Used by look-ahead planner to calculate optimal block velocities.
 */
typedef struct
{
    float entry_velocity;     // Block entry speed (mm/min)
    float peak_velocity;      // Maximum achievable speed (mm/min)
    float exit_velocity;      // Block exit speed (mm/min)
    float accel_distance;     // Distance spent accelerating (mm)
    float cruise_distance;    // Distance at constant speed (mm)
    float decel_distance;     // Distance spent decelerating (mm)
    float total_time;         // Total block execution time (sec)
} velocity_profile_t;

/*! \brief S-curve 7-segment timing profile
 *
 *  Pre-calculated segment durations for jerk-limited motion.
 *  Generated by look-ahead planner, consumed by motion controller.
 */
typedef struct
{
    float t1_jerk_accel;             // Segment 1: Jerk-limited accel ramp-up (sec)
    float t2_const_accel;            // Segment 2: Constant acceleration (sec)
    float t3_jerk_decel_accel;       // Segment 3: Jerk-limited accel ramp-down (sec)
    float t4_cruise;                 // Segment 4: Constant velocity cruise (sec)
    float t5_jerk_accel_decel;       // Segment 5: Jerk-limited decel ramp-up (sec)
    float t6_const_decel;            // Segment 6: Constant deceleration (sec)
    float t7_jerk_decel_decel;       // Segment 7: Jerk-limited decel ramp-down (sec)
    float peak_velocity;             // Maximum velocity reached (mm/min)
    float entry_velocity;            // Starting velocity (mm/min)
    float exit_velocity;             // Ending velocity (mm/min)
} scurve_timing_t;

/*! \brief Multi-axis coordination data
 *
