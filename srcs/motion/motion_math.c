...
void MotionMath_LoadDefaultSettings(void)
{
    ...
    // $110-$113: Max rate (mm/min) - Conservative for testing
    motion_settings.max_rate[AXIS_X] = 5000.0f;
    motion_settings.max_rate[AXIS_Y] = 5000.0f;
    motion_settings.max_rate[AXIS_Z] = 2000.0f;  // Z typically slower
    motion_settings.max_rate[AXIS_A] = 5000.0f;
    
    // $120-$123: Acceleration (mm/sec²)
    motion_settings.acceleration[AXIS_X] = 500.0f;
    motion_settings.acceleration[AXIS_Y] = 500.0f;
    motion_settings.acceleration[AXIS_Z] = 200.0f;  // Z typically slower
    motion_settings.acceleration[AXIS_A] = 500.0f;
    ...
    // Advanced parameters
    motion_settings.jerk_limit = 5000.0f;  // mm/sec³ - For S-curves
...
uint32_t MotionMath_FeedrateToOCRPeriod(float feedrate_mm_min, axis_id_t axis)
{
    ...
    // Clamp to 16-bit timer max (with safety margin)
    if (period > OCR_MAX_PERIOD)
    {
        period = OCR_MAX_PERIOD;
    }
    else if (period < 50)  // Minimum 50µs (20kHz max step rate)
    {
        period = 50;
    }
