...
{
    if (!grbl_initialized) return false;
...
{
    if (!grbl_initialized) return 0.0f;
...
{
    if (!grbl_initialized) return;
    
    for (int i = 0; i < SETTINGS_COUNT; i++) {
        // Skip unused setting numbers
        if (i >= 7 && i <= 9) continue;
        if (i >= 14 && i <= 19) continue;
        if (i >= 28 && i <= 29) continue;
        if (i >= 33 && i <= 99) continue;
        if (i >= 103 && i <= 109) continue;
        if (i >= 113 && i <= 119) continue;
        if (i >= 123 && i <= 129) continue;
        if (i >= 133) continue;
...
{
    if (!grbl_initialized || !command) return false;
...
void GRBL_SendStatusReport(void)
{
    ...
    switch (grbl_context.state) {
        case GRBL_STATE_IDLE: state_str = "Idle"; break;
        case GRBL_STATE_RUN: state_str = "Run"; break;
        case GRBL_STATE_HOLD: state_str = "Hold"; break;
        case GRBL_STATE_JOG: state_str = "Jog"; break;
        case GRBL_STATE_ALARM: state_str = "Alarm"; break;
        case GRBL_STATE_DOOR: state_str = "Door"; break;
        case GRBL_STATE_CHECK: state_str = "Check"; break;
        case GRBL_STATE_HOME: state_str = "Home"; break;
        case GRBL_STATE_SLEEP: state_str = "Sleep"; break;
    }
...
{
    if (!grbl_context.limits.soft_limits_enabled) return true;
...
{
    if (!grbl_context.limits.hard_limits_enabled) return true;
